...
def filter_by_correct(config, df):
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        for j, prompt_text in enumerate(batch_prompts):
            ...
            completions.append(completion.replace("\n", " ").strip())
        

        answers.extend(completions)

    score_card = [capital in answer for capital, answer in zip(df["capital"], answers)]
    df["model_correct"] = score_card
    df["model_response"] = answers
    return df
...
def find_critical_features(config, df):
    """Assumes df is already filtered so that the model natural gets the correct answer. For each puzzle, searches all features"""
